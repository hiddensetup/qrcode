...
class QrCodeGenerator extends Controller {
    ...
    public function index() {
        ...
        $_POST['background_color_transparency'] = isset($_POST['background_color_transparency']) && in_array($_POST['background_color_transparency'], range(0, 99)) ? (int) $_POST['background_color_transparency'] : 0;
        ...
        switch($_POST['type']) {
            ...
            case 'event':
                ...
                $_POST['event_start_datetime'] = (new \DateTime($_POST['event_start_datetime'], new \DateTimeZone($_POST['event_timezone'])))->setTimezone(new \DateTimeZone('UTC'))->format('Ymd\THis\Z');
                $_POST['event_end_datetime'] = empty($_POST['event_end_datetime']) ? null : (new \DateTime($_POST['event_end_datetime'], new \DateTimeZone($_POST['event_timezone'])))->setTimezone(new \DateTimeZone('UTC'))->format('Ymd\THis\Z');
                $_POST['event_first_alert_datetime'] = empty($_POST['event_first_alert_datetime']) ? null : (new \DateTime($_POST['event_first_alert_datetime'], new \DateTimeZone($_POST['event_timezone'])))->setTimezone(new \DateTimeZone('UTC'))->format('Ymd\THis\Z');
                $_POST['event_second_alert_datetime'] = empty($_POST['event_second_alert_datetime']) ? null : (new \DateTime($_POST['event_second_alert_datetime'], new \DateTimeZone($_POST['event_timezone'])))->setTimezone(new \DateTimeZone('UTC'))->format('Ymd\THis\Z');
            ...
            case 'pix':
                ...
                $data = '000201';
                $data .= '26460014BR.GOV.BCB.PIX';
                $data .= '01' . str_pad(mb_strlen($_POST['pix_payee_key']), 2, '0', STR_PAD_LEFT) . $_POST['pix_payee_key'];
                if($_POST['pix_description']) {
                    $data .= '02' . str_pad(mb_strlen($_POST['pix_description']), 2, '0', STR_PAD_LEFT) . $_POST['pix_description'];
                }
                $data .= '52040000';
                $data .= '5303986';
                $data .= '5406' . $_POST['pix_amount'];
                $data .= '5802BR';
                $data .= '59' . str_pad(mb_strlen($_POST['pix_payee_name']), 2, '0', STR_PAD_LEFT) . $_POST['pix_payee_name'];
                if($_POST['pix_transaction_id']) {
                    $data .= '621305' . str_pad(mb_strlen($_POST['pix_transaction_id']), 2, '0', STR_PAD_LEFT) . $_POST['pix_transaction_id'];
                }
                $data .= '63045664';
        ...
        if(($_POST['qr_code_foreground'] || $qr_code_foreground) && !isset($_POST['qr_code_foreground_remove'])) {
            ...
            $original_qr_group = $svg_object->getDocument()->getChild(1);
            ...
            $qr_group = $custom_svg_doc->getChild(3)->getChild(0);
